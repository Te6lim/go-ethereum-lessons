...
import (
	...
	"github.com/te6lim/go-ethereum-lessons/contracts/store"
)
...
func main() {
	...
	DeployingASmartContract(*client)
...
func SmartContractComilationAndABI() {
	// install solc
	// install abigen (for genetating an ABI)
	// solc --bin <path to smart contract> -o to build the sol file
	// run solc --abi <file path> -o build to generate abi
	// run abigen --abi=<abi file path> --pkg=<package name> --out=<output file path> to convert abi file into go file

	// in order to deploy the contract, we also need to cmpile the solidity smart contact to EVM bytecode. the bin file is required fo generating the deploy methods on the go contract file
	// solc --bin <smart contract file path> -o build
	//abigen --bin=<path to store new bin file> --abi=<path to abi file> --pkg=<package>  --out=<path to store go file>
}

func DeployingASmartContract(client ethclient.Client) {
	privateKey, err := crypto.HexToECDSA("fad9c8855b740a0b7ed4c221dbad0f33a83a49cad6b3fe8d5817ac83d38b6a19")
	if err != nil {
		log.Fatal(err)
	}
	...
	publicKeyECDSA, ok := publicKey.(*ecdsa.PublicKey)
	if !ok {
		log.Fatal("cannot assert type: publicKey is not type *ecdsa.PublicKey")
	}
	...
	address, tx, insance, err := store.DeployStore(auth, &client, input)
	if err != nil {
		log.Fatal(err)
	}
...
func LoadingASmartContract(client ethclient.Client) {
